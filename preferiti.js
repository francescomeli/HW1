...
function onJson2(json){
    ...
    for(let i in doc){
        ...
        for(let j in lista2){
            if(lista2[j].textContent===doc[i].Nome_piscina){
                tasto[j].textContent="Inserito nella sezione preferiti";
            }
        }
...
function trovaId(event){
    ...
    fetch("valutazioni_p.php?piscina="+identificativo).then(preferiti);
}
...
function elimina(event){
    ...
    fetch("rimozione.php?piscina="+identificativo).then(preferiti);
}


function preferiti(){
    fetch("caricamento_pref.php").then(onResponse).then(onJson2,onError);
}
preferiti();

function piscine(){
    fetch("caricamento_p.php").then(onResponse).then(onJson,onError);
}
piscine();